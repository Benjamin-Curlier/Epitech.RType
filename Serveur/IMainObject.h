...
{
public:
	IMainObject(sf::Vector2f pos,  Main_Object_type *t, std::vector<Missile_Type *> m, Explosion_Type *e) : IObject(pos)
	{
		this->_spec = t;
	...
	void fire(Missile_Type *m) 
	{
		for (int i = 0; i < this->_spec->_fire_pos.size(); i++)
			this->_missiles.push_back(new Missile(this->_pos + this->_spec->_fire_pos[i], m));
	}
	void explode() { this->_explosion = new Explosion(this->_pos + this->_spec->_center  - this->_explosion_type->_center, this->_explosion_type); }
